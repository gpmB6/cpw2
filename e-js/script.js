...
//ENDTIMER
	...
	function startGame(){
		...
		for(var i = 0; i < 8; i++){
			...
			card.style.left = (i % 4) === 0 ? 5 + "px" : 5 + ((i % 4) * 155) + "px";
		...
		if(flippedCards.length < 2){
			...
			if(flippedCards.length === 2){
				...
				if(flippedCards[0].childNodes[3].id === flippedCards[1].childNodes[3].id){
					...
					//verifica se o contador de acertos chegou a 4
					if(matches >= 4){
					//caso haja 4 acertos ou mais, chama a função que finaliza o jogo
	...
	function gameOver(){
		...
		document.getElementById("duracao").innerHTML = ss;
